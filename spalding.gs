function exportShopifyOrders() {
  ...
  orders.forEach(order => {
    ...
    const { includeOrder, hasRefundToday, refundedItems, fulfillmentYesterday, hasEverBeenFulfilled } = shouldIncludeOrder(order, yesterdayStr, CONFIG, apiOptions);
    
    if (!includeOrder) return;
    
    // Extract order data
    const orderData = extractOrderData(order, CONFIG, apiOptions, yesterdayStr, hasEverBeenFulfilled);
    
    // Process line items based on order type
    // BUSINESS RULES:
    // 1. Ordine spedito ieri -> Tutti gli articoli positivi sul file
    // 2. Rimborso ieri (su ordine già spedito) -> Solo rimborsi negativi sul file
    // 3. Mai processare rimborsi su ordini mai spediti
    
    const wasShippedYesterday = fulfillmentYesterday; // spedizione ieri
    const wasRefundedYesterday = hasRefundToday; // rimborso ieri
    
    if (wasShippedYesterday && !wasRefundedYesterday) {
      // Scenario 1: Ordine spedito ieri, nessun rimborso ieri
      // -> Processa tutti gli articoli come positivi
      processAllItems(order, {}, orderData, sheet); // Pass empty refundedItems
      Logger.log(`SPEDIZIONE: Processati tutti gli articoli per ordine ${order.name} (spedito ieri)`);
      
    } else if (!wasShippedYesterday && wasRefundedYesterday && hasEverBeenFulfilled) {
      // Scenario 2: Rimborso ieri su ordine precedentemente spedito
      // -> Processa solo i rimborsi come negativi
      processRefundedItems(order, refundedItems, orderData, sheet);
      Logger.log(`RIMBORSO: Processati solo rimborsi per ordine ${order.name} (rimborsato ieri, precedentemente spedito)`);
      
    } else if (wasShippedYesterday && wasRefundedYesterday) {
      // Scenario 3: Sia spedizione che rimborso ieri (caso raro)
      // -> Processa tutto: articoli positivi + rimborsi negativi
      processAllItems(order, refundedItems, orderData, sheet);
      Logger.log(`SPEDIZIONE+RIMBORSO: Processati articoli e rimborsi per ordine ${order.name} (spedito e rimborsato ieri)`);
      
    } else {
      // Scenario non previsto - logga per debug
      Logger.log(`SKIP: Ordine ${order.name} saltato - wasShippedYesterday:${wasShippedYesterday}, wasRefundedYesterday:${wasRefundedYesterday}, hasEverBeenFulfilled:${hasEverBeenFulfilled}`);
    }
  });
...
function fetchOrders(config, apiOptions) {
  try {
    ...
    startDate.setDate(startDate.getDate() - 180); // Ultimi 180 giorni
 ...
 * FIXED: Solo i rimborsi di ordini già spediti vengono processati
 */
function shouldIncludeOrder(order, yesterdayStr, config, apiOptions) {
  ...
  let hasEverBeenFulfilled = false; // NEW: Track if order was ever fulfilled
  
  if (order.fulfillments && order.fulfillments.length > 0) {
    hasEverBeenFulfilled = true; // Order has been fulfilled at some point
  ...
  // RULE: Solo rimborsi di IERI per ordini che sono stati già spediti in passato
  try {
    ...
    if (refundsData.refunds && refundsData.refunds.length > 0) {
      refundsData.refunds.forEach(refund => {
        ...
        // RULE: Include refund ONLY if:
        // 1. Refund was created yesterday
        // 2. Order was previously fulfilled (shipped at some point in the past)
        if (isRefundedYesterday && hasEverBeenFulfilled) {
          hasRefundToday = true;
          includeOrder = true;
          Logger.log(`Rimborso processato per ordine ${order.name} - ordine precedentemente spedito`);
        } else if (isRefundedYesterday && !hasEverBeenFulfilled) {
          // Log per debug: rimborso ignorato perché ordine mai spedito
          Logger.log(`Rimborso IGNORATO per ordine ${order.name} - ordine mai spedito`);
        }
        
        // Track refunded items for quantity calculations
        if (refund.refund_line_items && refund.refund_line_items.length > 0) {
          refund.refund_line_items.forEach(refundItem => {
            ...
            // Mark as refunded today only if refunded yesterday and order was ever fulfilled
            if (isRefundedYesterday && hasEverBeenFulfilled) {
  ...
  return { 
    ...
    hasEverBeenFulfilled // NEW: Return this info for further processing
  };
}

/**
 * Extracts relevant data from an order
 * UPDATED: Include hasEverBeenFulfilled information
 */
function extractOrderData(order, config, apiOptions, yesterdayStr, hasEverBeenFulfilled = false) {
  ...
  return {
    ...
    subtotalWithoutTax,
    fulfillmentYesterday,
    hasEverBeenFulfilled // NEW: Include this information
  };
}
